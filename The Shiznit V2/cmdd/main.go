...
func main() {
	...
	// طباعة رسالة تجهيز السيشنات بالإنكليزية
	fmt.Println("Preparing sessions...")

	stats := &insta.Stats{}
	clientCache := make(map[string]*insta.InstaClient)
	var cacheMu sync.Mutex

	// تجهيز كل السيشنات الموجودة في sessions.txt مسبقاً قبل تشغيل الواجهة
	for _, sess := range sessions {
		client := insta.NewInstaClient(sess, stats, getRandomProxy)
		if err := client.FetchSharedData(); err != nil {
			fmt.Printf("[Prep] Invalid session removed (CSRF failed): %v\n", err)
			_ = utils.RemoveFromFile("sessions.txt", sess)
			continue
		}
		if err := client.ProfileInfo(); err != nil {
			fmt.Printf("[Prep] Invalid session removed (Profile failed): %v\n", err)
			_ = utils.RemoveFromFile("sessions.txt", sess)
			continue
		}
		clientCache[sess] = client
	}

	// التأكد من وجود سيشنات صالحة بعد التجهيز
	validSessions, _ := utils.ReadFileLines("sessions.txt")
	if len(validSessions) == 0 {
		ui.ShowStandaloneAlert("تنبيه السيشنات", "جميع السيشنات المضافة غير صالحة، يرجى تحديث الملف وإعادة تشغيل البرنامج.")
		return
	}

	// مسح الشاشة بالكامل بعد تجهيز السيشنات
	clearConsole()
	...
	for i := 0; i < claimThreads; i++ {
		...
		go func(id int) {
			...
			for {
				...
				// حالة احتياطية في حال إدخال سيشن جديد أثناء التشغيل
				if client == nil {
				...
				if err == nil && success {
					...
					if !loaded {
						...
						dashboard.ShowAlert("تمت المطالبة بنجاح", fmt.Sprintf("تمت المطالبة باليوزر بنجاح: %s", target))
						showSessionsEmptyAlert(dashboard)
					}
				} else if err != nil {
					if insta.IsSessionInvalid(err) {
						dashboard.Log(fmt.Sprintf("[Claimer] session expired and removed: %s", session), "red")
						_ = utils.RemoveFromFile("sessions.txt", session)
						cacheMu.Lock()
						delete(clientCache, session)
						cacheMu.Unlock()
						showSessionsEmptyAlert(dashboard)
						continue
					}
